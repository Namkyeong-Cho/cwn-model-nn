...
def pyg_to_simplex_tree(edge_index: Tensor, size: int):
    ...
    """
    # print("size in pyg_to_simplex_tree : ", size, edge_index)
    st = gd.SimplexTree()
...
def convert_graph_dataset_with_rings(dataset, max_ring_size=7, include_down_adj=False,
                                     init_method: str = 'sum', init_edges=True, init_rings=True,
                                     n_jobs=1):
    print("convert_graph_dataset_with_rings")
    dimension = -1
    ...
    print("*" * 100)
    print("data.edge_attr", dataset[0].num_nodes)
    print("*"*100)
